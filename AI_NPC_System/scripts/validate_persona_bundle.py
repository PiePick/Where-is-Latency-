...
EXPECTED_EMOTIONS = {"Positive", "Negative", "Ambiguous", "Neutral"}
EXPECTED_RESPONSE_ACTS = {"QUESTION", "INFORM", "ACKNOWLEDGE", "DIRECTIVE", "EXPRESSIVE", "REJECT"}
EXPECTED_STYLE_TAGS = {"high-pitched", "playful", "energetic", "smug", "cute"}
EXPECTED_VARIANTS_PER_CELL = 5
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    expected_cell_count = len(EXPECTED_EMOTIONS) * len(EXPECTED_RESPONSE_ACTS) * len(EXPECTED_STYLE_TAGS)
    ...
    for cell in cells:
        ...
        if len(item_ids) != EXPECTED_VARIANTS_PER_CELL:
            failures.append(f"{cell_id}: item count {len(item_ids)} != {EXPECTED_VARIANTS_PER_CELL}")
    ...
    for item in items:
        ...
        audio_path = resolve_audio_path(manifest_path, str(item.get("audio_path") or ""))
